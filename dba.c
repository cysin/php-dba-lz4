...
static void php_dba_update(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	...
	DBA_WRITE_CHECK;
        ...
        output = (char *)emalloc(LZ4_compressBound(val_len) + sizeof(int));
        if (!output) {
            //memory allocation failed
            RETURN_FALSE;        
        }
        //add the vallen to the result
        memcpy(output, &val_len, sizeof(int));
        
        output_len = LZ4_compressHC(val, output + sizeof(int), val_len);
        //output_len = LZ4_compress(val, output + sizeof(int), val_len);
        if(output_len < 0) {
            RETURN_FALSE;
        }
        output_len += sizeof(int);
        
	if (info->hnd->update(info, key_str, key_len, output, output_len, mode TSRMLS_CC) == SUCCESS) {
                efree(output);
		DBA_ID_DONE;
		RETURN_TRUE;
	}
...
PHP_FUNCTION(dba_fetch)
{
	...
	if((val = info->hnd->fetch(info, key_str, key_len, skip, &len TSRMLS_CC)) != NULL) {
                ...
                * ****************************/            
                //get output_len
                memcpy(&output_len, val, sizeof(int));
                output = (char *)safe_emalloc(output_len, 1, 1);
                ...
                real_output_len = LZ4_uncompress_unknownOutputSize(val + sizeof(int), output,
                                                  len - sizeof(int), output_len);

                if (real_output_len <= 0 || real_output_len != output_len) {
                    RETURN_FALSE;
                }
                output[output_len] = 0;
                efree(val);
